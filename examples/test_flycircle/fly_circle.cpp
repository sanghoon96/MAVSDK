...
{   //==============================================================================================
    ...
    double point_x2 = center_lat + 0.0005, point_y2 = center_lon + 0.0005;  // second point
    double point_x3 = center_lat + 0.0005, point_y3 = center_lon - 0.0005;  // third point
    double point_x4 = center_lat         , point_y4 = center_lon - 0.0005;  // fourth point

    // 장애물 포인트, 임시로 포인트1과 포인트2 사이에 지정
    double point_x_obstacle1 = point_x1 + 0.5*(point_x2 - point_x1), point_y_obstacle1 = point_y1 + 0.5*(point_y2 - point_y1);
    // 장애물 포인트, 임시로 포인트2과 포인트3 사이에 지정
    double point_x_obstacle2 = point_x1 + 0.5*(point_x3 - point_x2), point_y_obstacle2 = point_y1 + 0.5*(point_y3 - point_y2);
    // 장애물 포인트, 임시로 포인트3과 포인트4 사이에 지정
    double point_x_obstacle3 = point_x1 + 0.5*(point_x4 - point_x3), point_y_obstacle3 = point_y1 + 0.5*(point_y4 - point_y3); 
    
    //기본 사각형 비행, point1과 point2 사이에 장애물
    make_mission_point(mission_items, point_x1, point_y1);  //사각형 point1
    ...
    make_mission_point(mission_items, point_x2, point_y2);  //사각형 point2
    double seta = atan((point_y2-point_y1_obstacle)/(point_x2-point_x1_obstacle));    // 진입각도 계산, radian
    collision_avoidance_point(mission_items, point_x_obstacle, point_y_obstacle, seta); // 장애물 회피

    make_mission_point(mission_items, point_x3, point_y3);  //사각형 point3
    double seta = atan((point_y3-point_y2_obstacle)/(point_x3-point_x2_obstacle));    // 진입각도 계산, radian
    collision_avoidance_point(mission_items, point_x_obstacle, point_y_obstacle, seta); // 장애물 회피

    make_mission_point(mission_items, point_x4, point_y4);  //사각형 point4
